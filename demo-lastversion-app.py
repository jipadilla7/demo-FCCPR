...
if not df_filtrado.empty:
    fig_modalidad = px.bar(df_filtrado, x="Modalidad", title="Distribución por Modalidad")
    st.plotly_chart(fig_modalidad)
    
    fig_sexo = px.bar(df_filtrado, x="Sexo", title="Distribución por Género")
    st.plotly_chart(fig_sexo)
    
    fig_ciudad = px.bar(df_filtrado, x="Ciudad", title="Distribución por Ciudad")
